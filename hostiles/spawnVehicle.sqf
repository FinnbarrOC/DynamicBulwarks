...
for "_i" from 0 to (floor (attkWave / 10) - 1) do {
	...
	while {!_found} do {
		...
		if (isClass _checked_veh) then { // CHECK IF THE SELECTED ENTRY IS A CLASS
			...
			_unsCheck = toLower(configname _checked_veh select [0,4]);
			if (_vehclass == _vehClass && _scope != 0 && _simulation_paracheck != "parachute" && _classname != "O_MBT_02_arty_F" && _actual_vehclass == "tank" && _unsCheck == "uns_") exitWith {
